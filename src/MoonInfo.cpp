...
double getJulianDay(const std::tm& utc_tm) {
    ...
    double hour = utc_tm.tm_hour + utc_tm.tm_min / 60.0 + utc_tm.tm_sec / 3600.0;
...
public:
    ...
    MoonInfo() {
        ...
        const auto user_coords = fetchCoordinates();
        double observer_longitude = user_coords.first;
        double observer_latitude = user_coords.second;
...
private:
    [[nodiscard]] std::pair<double, double> fetchCoordinates() {
        ...
        if (res && res->status == 200) {
            try {
                ...
                if (data.contains("lon") && data.contains("lat")) {
                    return {data["lon"].get<double>(), data["lat"].get<double>()};
                }
            } catch (const json::exception& e) {
                std::cerr << "JSON parsing failed for IP-API response: " << e.what() << std::endl;
            }
        }
        std::cerr << "Failed to fetch coordinates from ip-api.com. Defaulting to (0.0, 0.0)." << std::endl;
        return {0.0, 0.0};
    }
    ...
    double getGMST(double JD) {
        ...
        double GMST_deg = 280.46061837 + 360.98564736629 * (JD - JD_2000_0) + 0.000387933 * T * T - T * T * T / 38710000.0;
    ...
    double getObliquityAndNutation(double JD, double& delta_psi, double& delta_epsilon) {
        ...
        double L_dash = normalizeDegrees(218.31221 + 481267.881 * T);
        double M = normalizeDegrees(357.5291 + 35999.0503 * T);
        double F = normalizeDegrees(93.27191 + 483202.0175 * T);
        double Omega = normalizeDegrees(125.04452 - 1934.13626 * T);

        L_dash = degreesToRadians(L_dash);
        M = degreesToRadians(M);
        F = degreesToRadians(F);
        Omega = degreesToRadians(Omega);

        delta_psi = (-17.200 * sin(Omega) - 1.319 * sin(2 * L_dash) - 0.227 * sin(2 * F) + 0.206 * sin(2 * Omega)) / 3600.0;
        delta_epsilon = (9.202 * cos(Omega) + 0.573 * cos(2 * L_dash) + 0.098 * cos(2 * F) - 0.090 * cos(2 * Omega)) / 3600.0;
    ...
    SolarCoords getSolarCoordinates(double JD) {
        ...
        double M = normalizeDegrees(357.5291092 + 0.985600283 * D);

        double C = 1.9148 * sin(degreesToRadians(M)) + 0.0200 * sin(degreesToRadians(2 * M)) + 0.0003 * sin(degreesToRadians(3 * M));

        double L0 = normalizeDegrees(280.46646 + 0.98564736 * D);
        
        double lambda_sun = normalizeDegrees(L0 + C);

        double R = 1.00014 - 0.01671 * cos(degreesToRadians(M)) - 0.00014 * cos(degreesToRadians(2 * M));
    ...
    LunarCoords getLunarCoordinates(double JD) {
        ...
        double sum_lon = 0.0;
        sum_lon += 6.28875 * sin(degreesToRadians(M_moon));
        sum_lon += 1.27400 * sin(degreesToRadians(2 * D_angle));
        sum_lon += 0.65830 * sin(degreesToRadians(2 * D_angle));
        sum_lon -= 0.18580 * sin(degreesToRadians(M_sun));
        sum_lon -= 0.11430 * sin(degreesToRadians(2 * F + M_moon));
        sum_lon += 0.05880 * sin(degreesToRadians(2 * D_angle - M_moon));
        sum_lon += 0.05730 * sin(degreesToRadians(2 * D_angle + M_moon));
        sum_lon += 0.05330 * sin(degreesToRadians(2 * F + M_sun));
        sum_lon += 0.04630 * sin(degreesToRadians(2 * F - M_moon));
        sum_lon += 0.04100 * sin(degreesToRadians(2 * D_angle - M_sun));
        sum_lon += 0.03040 * sin(degreesToRadians(M_moon + M_sun));
        sum_lon += 0.01530 * sin(degreesToRadians(2 * D_angle - 2 * F));
        sum_lon += 0.01130 * sin(degreesToRadians(M_moon - M_sun));

        double lambda_moon = normalizeDegrees(L_moon + sum_lon);

        double sum_lat = 0.0;
        sum_lat += 5.12820 * sin(degreesToRadians(F));
        sum_lat += 0.28060 * sin(degreesToRadians(M_moon + F));
        sum_lat += 0.27770 * sin(degreesToRadians(F - M_moon));
        sum_lat += 0.17320 * sin(degreesToRadians(M_sun + F));
        sum_lat += 0.05540 * sin(degreesToRadians(2 * D_angle + F));
        sum_lat += 0.04620 * sin(degreesToRadians(2 * D_angle - F));
        sum_lat += 0.03260 * sin(degreesToRadians(2 * D_angle - M_moon + F));

        double beta_moon = sum_lat;

        double R_moon = 385000.0 - 20905.0 * cos(degreesToRadians(M_moon));
        R_moon -= 3699.0 * cos(degreesToRadians(2 * D_angle - M_moon));
        R_moon -= 2956.0 * cos(degreesToRadians(2 * D_angle));
        R_moon -= 569.0 * cos(degreesToRadians(2 * F));
        R_moon += 246.0 * cos(degreesToRadians(2 * D_angle - 2 * F));
        R_moon += 209.0 * cos(degreesToRadians(M_moon + M_sun));
        R_moon += 105.0 * cos(degreesToRadians(M_sun));
        R_moon -= 103.0 * cos(degreesToRadians(M_moon - M_sun));
        R_moon -= 57.0 * cos(degreesToRadians(M_moon + 2 * D_angle));
        R_moon -= 48.0 * cos(degreesToRadians(M_moon + 2 * F));
        R_moon += 46.0 * cos(degreesToRadians(2 * D_angle - M_sun - M_moon));
        R_moon += 38.0 * cos(degreesToRadians(2 * D_angle + M_moon));
        R_moon -= 30.0 * cos(degreesToRadians(M_moon + F + 2 * D_angle));
        R_moon -= 24.0 * cos(degreesToRadians(M_moon - 2 * D_angle));
        R_moon -= 22.0 * cos(degreesToRadians(2 * D_angle - F));
        R_moon += 15.0 * cos(degreesToRadians(M_moon - 2 * F));
        R_moon -= 13.0 * cos(degreesToRadians(M_moon + 2 * D_angle + F));
        R_moon -= 12.0 * cos(degreesToRadians(M_sun + 2 * D_angle));
        R_moon += 10.0 * cos(degreesToRadians(M_sun - 2 * F));
        R_moon += 8.0 * cos(degreesToRadians(2 * D_angle + M_sun + F));
        R_moon += 7.0 * cos(degreesToRadians(M_moon + F));
        R_moon -= 6.0 * cos(degreesToRadians(2 * D_angle + M_sun - F));
        R_moon -= 5.0 * cos(degreesToRadians(M_moon + 2 * F - 2 * D_angle));
        R_moon -= 4.0 * cos(degreesToRadians(M_moon - F + 2 * D_angle));
        R_moon += 4.0 * cos(degreesToRadians(M_moon + M_sun + 2 * D_angle));
        R_moon -= 4.0 * cos(degreesToRadians(2 * D_angle - 2 * M_sun));
        R_moon -= 3.0 * cos(degreesToRadians(M_moon - M_sun - 2 * D_angle));
        R_moon -= 3.0 * cos(degreesToRadians(M_sun - F));
        R_moon -= 3.0 * cos(degreesToRadians(2 * F + 2 * D_angle));
        R_moon += 3.0 * cos(degreesToRadians(M_moon + M_sun - F));
        R_moon -= 3.0 * cos(degreesToRadians(2 * F + M_sun));
        R_moon -= 3.0 * cos(degreesToRadians(2 * D_angle - M_moon - M_sun));
        R_moon += 3.0 * cos(degreesToRadians(M_moon - M_sun + 2 * D_angle));
        R_moon += 3.0 * cos(degreesToRadians(M_moon + M_sun + F));
        R_moon -= 3.0 * cos(degreesToRadians(M_moon + F - 2 * D_angle));
        R_moon -= 2.0 * cos(degreesToRadians(2 * D_angle - M_moon - F));
    ...
    void calculatePhaseAndIllumination(double JD) {
        ...
        constexpr double GIBBOUS_MAX = 0.99;
        ...
        } else if (illum_fraction < GIBBOUS_MAX) {
    ...
    double calculateAltitude(double JD_utc, double longitude_deg, double latitude_deg) {
        ...
        double geocentric_ra_rad = normalizeRadians(atan2(sin(true_ecl_lon_moon_rad) * cos(true_obliquity_rad) - tan(true_ecl_lat_moon_rad) * sin(true_obliquity_rad), cos(true_ecl_lon_moon_rad)));
        double geocentric_dec_rad = asin(sin(true_ecl_lat_moon_rad) * cos(true_obliquity_rad) + cos(true_ecl_lat_moon_rad) * sin(true_obliquity_rad) * sin(true_ecl_lon_moon_rad));

        double lat_rad = degreesToRadians(latitude_deg);
        double lon_rad = degreesToRadians(longitude_deg);

        double gmst_deg = getGMST(JD_utc);
        double gmst_rad = degreesToRadians(gmst_deg);
        ...
        double sin_h = sin(topocentric_dec_rad) * sin_phi_prime + cos(topocentric_dec_rad) * cos_phi_prime * cos(lha_topocentric_rad);
    ...
    double refineRiseSetTime(double JD_interval_start, double JD_interval_end, double longitude_deg, double latitude_deg, double target_alt_deg) {
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            double alt_start = calculateAltitude(JD_interval_start, longitude_deg, latitude_deg);
            if ((alt_start - target_alt_deg) * (alt_at_mid - target_alt_deg) < 0) {
    ...
    void calculateRiseAndSetTimes(double JD_utc_now, double longitude_deg, double latitude_deg) {
        ...
        if (rise_found) {
            ...
            double alt_at_next_local_midnight = calculateAltitude(local_midnight_tomorrow_jd - 0.0001, longitude_deg, latitude_deg);
            if (alt_at_local_midnight > HORIZON_ALT_DEG && alt_at_next_local_midnight > HORIZON_ALT_DEG) {
                riseTimeString = "Always Above Horizon";
            } else if (alt_at_local_midnight < HORIZON_ALT_DEG && alt_at_next_local_midnight < HORIZON_ALT_DEG) {
